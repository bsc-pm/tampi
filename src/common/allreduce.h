...
void allreduce( const void *sendbuf, void *recvbuf, IntType count,
            DataType datatype, OpType op, CommType comm, ErrType *ierror )
{
    print::dbg( "[MPI Async. Overload Library] Intercepted MPI_Reduce" );

    shared_pointer<TicketType> waitCond = 
            iallreduce( sendbuf, recvbuf, count, datatype, op, comm );
