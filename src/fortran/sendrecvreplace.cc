...
extern "C" {
	...
	void mpi_unpack_(MPI3CONST void *inbuf, MPI_Fint *insize, MPI_Fint *position, void *outbuf, MPI_Fint *outcount, MPI_Fint *datatype, MPI_Fint *comm, MPI_Fint *err);
			...
			MPI_Fint *source, MPI_Fint *recvtag, MPI_Fint *comm, MPI_Fint *status, MPI_Fint *err)
	{
		if (Fortran::Environment::isEnabled()) {
			...
			if (*source == MPI_PROC_NULL || *dest == MPI_PROC_NULL || *count == 0) {
				...
				if (*err == MPI_SUCCESS && alloc_err == MPI_SUCCESS) {
					int position = 0;
					mpi_unpack_(helperbuf, &packed_size, &position, buf, count, datatype, comm, err);
				}
			...
			static Fortran::mpi_sendrecv_replace_t *symbol = (Fortran::mpi_sendrecv_replace_t *) Symbol::loadNextSymbol(__func__);
