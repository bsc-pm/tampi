...
* `TAMPI_POLLING_FREQUENCY` (default `500` us): The polling frequency in which the in-flight MPI requests are
...
Notice that the task leverages a CPU while running. By default, this task runs every `500` microseconds, but
this value may be too high in communication-intensive applications. This envar is only considered when running
with OmpSs-2 2020.06 or newer.
